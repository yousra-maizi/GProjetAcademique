...
public class ConnectionFactory {
    ...
    private static String     activeDbName     = null;
    ...
    // connexion vers n'importe quelle BD — utilisée pour consulter archives
    public static Connection getConnection(String dbName) throws ConnectionException {
        return createConnection(dbName);
    }
    // connexion au serveur MySQL sans BD — utilisée pour CREATE DATABASE
    ...
    public static Connection getActiveConnection() throws ConnectionException {
        if (activeConnection == null) {
            throw new ConnectionException(
                    "Aucune connexion active — appelez initialize() d'abord"
            );
    ...
    private static void updateConfig(String newDbName) throws ConfigException {
        try {
            String path = ConnectionFactory.class
                    .getProtectionDomain()
                    .getCodeSource()
                    .getLocation()
                    .toURI()
            ...
            System.out.println("📄 config.properties path: " + file.getAbsolutePath());
            ...
            System.out.println("✅ config.properties mis à jour: " + newDbName);

        } catch (Exception e) {
            throw new ConfigException(
                    "Impossible de mettre à jour config.properties", e
            );
        }
    }
    private static Properties loadProps() throws ConfigException {
        try {
            ...
            if (input == null) {
                throw new ConfigException("config.properties introuvable dans resources");
            }
            Properties props = new Properties();
            props.load(input);
            return props;
